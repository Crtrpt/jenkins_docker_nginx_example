    ...
    stages {
        ...
        stage('构建docker镜像') {
         ...
         stage('更新nginx配置文件 和重启nginx') {
            steps {
                 sh 'cp nginx.conf /opt/nginx/conf/jenkins_docker_example.conf'
            }
        }
         stage('销毁老的docker容器') {
            steps {
                 sh 'docker  rm -f  `docker ps  | grep jenkins_docker_example:jenkins_docker_example | cut -c 1-12` | true'
            }
        }
        stage('启动docker容器') {
            steps {
